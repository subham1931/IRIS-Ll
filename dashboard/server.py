"""
dashboard/server.py — IRIS Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "IRIS Uploads",
        Path.home() / "Documents" / "IRIS Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"IRIS Dashboard Port {port}"
        prog_rule  = "IRIS Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="iris_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until IRIS is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return ((certs / "iris.key").exists() and (certs / "iris.crt").exists()) or ((certs / "jarvis.key").exists() and (certs / "jarvis.crt").exists())
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in IRIS to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('iris_token','{tok}');
  sessionStorage.setItem('iris_key','{key}');
  localStorage.setItem('iris_device_token','{dev_tok}');
...
<p>Connecting to IRIS…</p>
    ...
    async def _serve_alias(self) -> None:
        ...
        certs = BASE_DIR / "config" / "certs"
        ssl_key  = certs / "iris.key" if (certs / "iris.key").exists() else certs / "jarvis.key"
        ssl_cert = certs / "iris.crt" if (certs / "iris.crt").exists() else certs / "jarvis.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in IRIS UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
