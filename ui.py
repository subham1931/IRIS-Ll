...
_DEFAULT_W, _DEFAULT_H = 1180, 700
_MIN_W,     _MIN_H     = 1000, 580
_LEFT_W  = 340
_RIGHT_W = 340
...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "I.R.I.S", parent=None):
    ...
    def paintEvent(self, _):
        ...
        # IRIS's own voice while speaking). Falls back to a gentle idle
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "iris"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("iris:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for IRIS", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure I.R.I.S. before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="IRIS", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "IRIS"
...
class AudioDeviceOverlay(_HudOverlay):
    """Choose which microphone IRIS listens to and which speakers it uses.
    ...
    is plugged in. 'IRIS can't hear me' is usually 'IRIS is listening to the
    webcam'."""
    ...
    def __init__(self, parent=None):
        ...
        self._in_box  = _row("MICROPHONE — what IRIS hears you with",
                             "input", get_input_device())
        lay.addSpacing(4)
        self._out_box = _row("SPEAKERS — what IRIS talks through",
...
class MemoryOverlay(_HudOverlay):
    """Everything IRIS has stored about you, and when it learned it.
    ...
    def _rebuild(self):
        ...
        hdr = QLabel("🧠  WHAT IRIS REMEMBERS")
        hdr.setFont(QFont("Courier New", 12, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.PRI}; background: transparent;")
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Iris actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — IRIS ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "IRIS").strip()
        ...
        self.on_interrupt      = None   # callable: () -> None — stop IRIS mid-speech
        self.on_voice_change   = None   # callable: () -> None — rebuild session with new voice
        self.on_audio_device_change = None  # callable: () -> None — reopen audio streams
        self._confirm_overlay  = None   # live ConfirmBanner, if one is on screen
        self.get_plugins       = None   # callable: () -> list[dict], set by IrisLive
        self.get_plugin_settings = None # callable: () -> list[dict] settings schemas, set by IrisLive
        self.on_wake_toggle    = None   # callable: (enable: bool) -> str, set by IrisLive
        ...
        from memory.config_manager import get_brief_enabled as _gbe, get_sfx_enabled as _gsfx
        self._update_brief_btn(_gbe())
        self._update_sfx_btn(_gsfx())
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_iris_icon(out_path: Path) -> bool:
        """
        Render an IRIS arc-reactor icon at 4× resolution and downsample
        ...
        def _render(sz: int) -> PIL.Image.Image:
            ...
            for i in range(6):
                ...
                d.line([x1, y1, x2, y2], fill=(*DIM, 200), width=spoke_w)

            # ── inner core glow (diffused circle) ─────────────────────────
            glow_sz = int(R * 0.55)
            glow_img = PIL.Image.new("RGBA", (S, S), (0, 0, 0, 0))
            gd = PIL.ImageDraw.Draw(glow_img)
            gd.ellipse([cx-glow_sz, cy-glow_sz, cx+glow_sz, cy+glow_sz],
                       fill=(*GLOW, 120))
            glow_img = glow_img.filter(PIL.ImageFilter.GaussianBlur(radius=S//16))
            img = PIL.Image.alpha_composite(img, glow_img)
            d   = PIL.ImageDraw.Draw(img)
            ...
            R4 = max(2, int(R * 0.16))
            d.ellipse([cx-R4, cy-R4, cx+R4, cy+R4],
                      fill=(*WHITE, 255))

            # Downsample to target size with lanczos filter for crisp edges
        ...
        try:
            ...
            # Save largest icon with smaller ones embedded as multi-res .ico
            images[-1].save(
                out_path,
                format="ICO",
            ...
            print(f"[Icon] Failed to build icon: {e}")
            return False

    def _create_lnk_windows(self, lnk: str, target: str, args: str,
                            work_dir: str, icon_loc: str) -> None:
        """
        Create a Windows .lnk shortcut with NO CMD window at launch.

        Attempts pywin32 (Dispatch('WScript.Shell')) first — fast, in-process,
        zero subprocess. Falls back to generating a temporary .vbs script run
        with wscript.exe (always present on Windows, GUI-subsystem so it never
        creates a console window).

        Why NOT PowerShell: PowerShell is a console-mode executable and always
        creates a brief black CMD window when launched from GUI Python.
        ...
        try:
            ...
            sc.Description      = "I.R.I.S AI Assistant"
    ...
    def _create_desktop_shortcut(self):
        """
        Creates a desktop shortcut for the current platform:
          - Windows : I.R.I.S.lnk (pythonw.exe target — no CMD window)
          - macOS   : I.R.I.S.app (bundled launcher app — no Terminal window)
          - Linux   : I.R.I.S.desktop (Terminal=false desktop entry)
        Uses the embedded arc-reactor icon.
        """
        ...
        ico_path = Path(__file__).resolve().parent / "config" / "iris.ico"
        if not ico_path.exists():
            self._build_iris_icon(ico_path)
        ...
        try:
            ...
            # ── Windows ───────────────────────────────────────────────────────
            if _os == "Windows":
                # Clean up old JARVIS shortcut if present
                old_lnk = desktop / "J.A.R.V.I.S.lnk"
                if old_lnk.exists():
                    try:
                        old_lnk.unlink(missing_ok=True)
                    except Exception:
                        pass
                pythonw  = python.parent / "pythonw.exe"
                target   = str(pythonw if pythonw.exists() else python)
                lnk      = str(desktop / "I.R.I.S.lnk")
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                # Clean up old JARVIS app bundle if present
                old_app = desktop / "J.A.R.V.I.S.app"
                if old_app.exists():
                    try:
                        _shutil.rmtree(old_app, ignore_errors=True)
                    except Exception:
                        pass
                app     = desktop / "I.R.I.S.app"
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '<string>com.iris.assistant</string>\n'
                    '  <key>CFBundleName</key><string>I.R.I.S</string>\n'
            ...
            # ── Linux — .desktop file (Terminal=false, no console) ────────────
                ...
                # Clean up old JARVIS desktop file if present
                old_desk = desktop / "J.A.R.V.I.S.desktop"
                if old_desk.exists():
                    try:
                        old_desk.unlink(missing_ok=True)
                    except Exception:
                        pass
                icon_line = f"Icon={png_path}\n" if png_path.exists() else ""
                desk = desktop / "I.R.I.S.desktop"
                desk.write_text(
                    "[Desktop Entry]\n"
                    "Name=I.R.I.S\n"
                    f"Exec={python} {script}\n"
    ...
    def _build_header(self) -> QWidget:
        ...
        left_col.addWidget(_badge(APP_VERSION, C.PRI_DIM))
        ...
        _sub_text = ("Intelligent Response & Interface System"
                     if _disp in ("IRIS", "I.R.I.S", "I.R.I.S.")
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        for txt, col in [
            ...
            lbl.setStyleSheet(
                ...
                f"border: 1px solid {C.BORDER_A}; border-radius: 3px; padding: 6px 4px;"
            )
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        self._sfx_btn = QPushButton()
        self._sfx_btn.setFixedHeight(26)
        self._sfx_btn.setFont(QFont("Courier New", 7))
        self._sfx_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._sfx_btn.clicked.connect(self._toggle_sfx)
        lay.addWidget(self._sfx_btn)
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("IRIS Mark-LIII", C.PRI_DIM))
        return w
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "IRIS_AI")
                    return True
                ...
                return ((Path.home() / "Library" / "LaunchAgents"
                        / "com.iris.assistant.plist").exists()
                        or (Path.home() / "Library" / "LaunchAgents"
                        / "com.jarvis.assistant.plist").exists())
            else:
                return ((Path.home() / ".config" / "autostart" / "iris.desktop").exists()
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    try:
                        winreg.DeleteValue(reg, "IRIS_AI")
                    except FileNotFoundError:
                        pass
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "IRIS_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                old_plist = plist_dir / "com.jarvis.assistant.plist"
                old_plist.unlink(missing_ok=True)
                plist = plist_dir / "com.iris.assistant.plist"
                if currently_on:
                ...
                old_desk = desk_dir / "jarvis.desktop"
                old_desk.unlink(missing_ok=True)
                desk = desk_dir / "iris.desktop"
                if currently_on:
    ...
    def _toggle_sfx(self):
        from memory.config_manager import get_sfx_enabled, save_sfx_enabled
        new_val = not get_sfx_enabled()
        save_sfx_enabled(new_val)
        self._update_sfx_btn(new_val)
        if new_val:
            from core import audio_fx
            audio_fx.play_complete()

    def _update_sfx_btn(self, enabled: bool):
        if not hasattr(self, '_sfx_btn'):
            return
        if enabled:
            self._sfx_btn.setText("🔊  AUDIO FX: ON")
            self._sfx_btn.setStyleSheet(f"""
                QPushButton {{
                    background: #001a18; color: {C.PRI};
                    border: 1px solid {C.PRI_DIM}; border-radius: 3px;
                }}
                QPushButton:hover {{ background: {C.PRI_GHO}; }}
            """)
        else:
            self._sfx_btn.setText("🔇  AUDIO FX: OFF")
            self._sfx_btn.setStyleSheet(f"""
                QPushButton {{
                    background: transparent; color: {C.TEXT_DIM};
                    border: 1px solid {C.BORDER}; border-radius: 3px;
    ...
    def _wake_state(self) -> dict:
        ...
        if self.wake_get_state:
            try:
                ...
                pass
        # Before IrisLive has wired its callback (drawer built at startup).
    ...
    def _toggle_wake_word(self):
        ...
        if not st["ready"]:
            ...
            return
        # Already downloaded → just flip enabled/disabled through IrisLive.
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "IRIS") or "IRIS",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = "",
        ...
        self._assistant_name = name.strip() or "IRIS"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — {APP_VERSION}")
        self._title_lbl.setText(display)
        if display in ("IRIS", "I.R.I.S", "I.R.I.S."):
            self._sub_lbl.setText("Intelligent Response & Interface System")
        else:
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "IRIS") or "IRIS"
