...
import warnings
# Silence sounddevice / NumPy 2.x in-place shape assignment deprecation warnings
...
# (screen_process, close_camera, save_memory, manage_monitor, shutdown_iris,
# system_status).
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are IRIS, an advanced personal AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    # core.action_loader (see IrisLive.__init__).
    {
        "name": "system_status",
        "description": (
    ...
    },
    {
        ...
        "description": (
            ...
            "IRIS checks these topics once a day and alerts the user when there is a new development. "
    ...
    },
    {
        ...
        "description": (
            ...
            "close the assistant, say goodbye, or stop Iris. "
...
class IrisLive:
    def __init__(self, ui: IrisUI):
        self.ui             = ui
        self._asst_name     = "IRIS"   # updated each session from config
        self.session              = None
    ...
    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask IRIS to
    ...
    def interrupt(self) -> None:
        """Stop IRIS mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[IRIS] ✋ Interrupted — {drained} audio chunks discarded")
        self.set_speaking(False)
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "IRIS").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "IRIS"
            _user_name = ""
        ...
        cfg = dict(
            ...
            # window — IRIS can stay in one conversation for hours
        ...
        if self._enhanced_live:
            # Proactive audio: IRIS stays silent when speech isn't addressed
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[IRIS] 📤 {name} → {str(result)[:80]}")
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _listen_audio(self):
        print("[IRIS] 🎤 Mic started")
        loop = asyncio.get_event_loop()

        def callback(indata, frames, time_info, status):
            # ── Wake-word gate ───────────────────────────────────────────────
            # While asleep, the mic audio NEVER goes to Gemini (nothing is
            # streamed, so IRIS can't respond to speech not addressed to it and
            ...
            if self._wake_enabled and not self._awake:
                ...
                return
            with self._speaking_lock:
                iris_speaking = self._is_speaking
            if not iris_speaking and not self.ui.muted and not self._phone_active:
                data = indata.tobytes()
                def _safe_put(item):
                    try:
        ...
        try:
            ...
            if _mic_dev is not None:
                print(f"[IRIS] 🎤 Input device: {_mic_name}")
            try:
                ...
                if _mic_dev is None:
                    raise
                print(f"[IRIS] ⚠️  Mic '{_mic_name}' failed: {_e} — using default")
                self.ui.write_log(
                    ...
                    if _sru is not None:
                        if getattr(_sru, "resumable", False) and getattr(_sru, "new_handle", None):
                            if self._resume_handle is None:
                                print("[IRIS] 🔗 Session resumption armed")
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until IRIS finishes speaking the answer
                                ...
                                async def _cam_close():
                                    ...
                                    self.ui.stop_camera_stream()
                                quarry = asyncio.create_task(_cam_close())

                    if response.tool_call:
                        fn_responses = []
                        for fc in response.tool_call.function_calls:
                            print(f"[IRIS] 📞 {fc.name}")
    ...
    async def _play_audio(self):
        ...
        try:
            ...
            if _spk_dev is None:
                raise
            print(f"[IRIS] ⚠️  Output device '{_spk_name}' failed: {_e} — using default")
        ...
        try:
            while True:
                ...
                # Drive the HUD waveform from IRIS's own voice while speaking.
                try:
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session and self._awake:
                # Don't interrupt if user spoke recently or IRIS is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
        ...
        while True:
            try:
                ...
                if self.session:
                    ...
                    # has no desktop WAKE button — so it wakes IRIS if asleep.
                    if self._wake_enabled and not self._awake:
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    if self._wake_enabled:
                        ...
                        self._awake = False
                        self.ui.set_state("SLEEPING")
                        self.ui.write_log("SYS: IRIS online — sleeping. Say 'Hey Jarvis' to wake me.")
                    else:
                        self._awake = True
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: IRIS online.")
                ...
                if _is_reconnect_signal(e):
                    print("[IRIS] Voluntary reconnect requested.")
                ...
                if _resumed_with and (
                    ...
                    print("[IRIS] 🔗 Resumption handle rejected — starting a fresh session")
                ...
                err_str = str(e)
                print(f"[IRIS] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    print("[IRIS] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
            ...
            print(f"[IRIS] Reconnecting in {delay}s...")
