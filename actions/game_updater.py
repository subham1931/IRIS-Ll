...
def _schedule_mac(hour: int, minute: int) -> str:
    ...
    plist_path  = plist_dir / "com.iris.gameupdater.plist"
    script_path = Path(__file__).resolve()
    plist_content = f"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
  "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
    <key>Label</key><string>com.iris.gameupdater</string>
    ...
    try:
        # Clean up legacy jarvis plist if present
        old_plist = plist_dir / "com.jarvis.gameupdater.plist"
        if old_plist.exists():
...
def _cancel_scheduled_update() -> str:
    if is_windows():
        r1 = subprocess.run(
            ["schtasks", "/Delete", "/TN", "IRIS_GameUpdater", "/F"],
            capture_output=True, text=True, **_CNW
        )
        r2 = subprocess.run(
    ...
    if is_mac():
        cancelled = False
        for name in ("com.iris.gameupdater.plist", "com.jarvis.gameupdater.plist"):
    ...
    try:
        existing = subprocess.run(["crontab", "-l"], capture_output=True, text=True)
        lines    = [l for l in existing.stdout.splitlines()
                    if "IRIS_GameUpdater" not in l and "JARVIS_GameUpdater" not in l]
...
def _get_schedule_status() -> str:
    if is_windows():
        for tn in ("IRIS_GameUpdater", "JARVIS_GameUpdater"):
    ...
    try:
        ...
        if "IRIS_GameUpdater" in result.stdout or "JARVIS_GameUpdater" in result.stdout:
            for line in result.stdout.splitlines():
                if "IRIS_GameUpdater" in line or "JARVIS_GameUpdater" in line:
