...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".iris_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".iris_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using IRIS profile")
                iris = str(Path.home() / ".iris_profiles" / "firefox_iris")
                Path(iris).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(iris, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".iris_profiles" / "safari")
        ...
        # automation). Fall back to a persistent IRIS automation profile —
        # accounts logged in here once stay logged in on later sessions too.
        iris_profile = str(Path.home() / ".iris_profiles" / self.browser_name)
        Path(iris_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with IRIS profile: {iris_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(iris_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with IRIS profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "iris_screenshot.png")
